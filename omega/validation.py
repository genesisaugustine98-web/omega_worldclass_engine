...
def validate(df: pd.DataFrame, timeframe_minutes=30, require_spread=False, max_robust_return_z=25.0, raise_on_failure=True):
    ...
    checks.append(Check(1,"schema", required.issubset(df.columns), f"columns={list(df.columns)}"))
    ...
    nums=df[["open","high","low","close"]]
    ohlc=(nums.gt(0).all().all() and np.isfinite(nums).all().all() and
          (df.high >= df[["open","close","low"]].max(axis=1)).all() and
          (df.low <= df[["open","close","high"]].min(axis=1)).all())
    z=_robust_z(np.log(df.close).diff()).abs(); outliers=int((z > max_robust_return_z).sum())
